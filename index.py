...
def sort_and_clean_csv(filename):
    try:
        ...
        logging.warning(f"⚠️  File '{filename}' not found. Will be created upon scraping.")
        return
...
def parse_user_details(html_content, user_id):
    ...
    if not user_info:
        return {'User ID': user_id, 'Enkord account full name': 'N/A', 'Registered': 'N/A', 'Accounts in games': 'N/A'}
...
def write_to_csv(user_details, filename):
    try:
        ...
        with open(filename, 'a', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['User ID', 'Enkord account full name', 'Registered', 'Accounts in games']
...
def run_missing_ids_scraper(filename):
    ...
    missing_user_ids = [uid for uid in range(start_id, end_id + 1) if uid not in existing_user_ids]
...
def recheck_anonymous_user(user_id):
    ...
    if user_details and not user_details.get('Enkord account full name', '').lower().startswith("anonymous#"):
        logging.info(f"✅ Name updated for ID {user_id}: {user_details['Enkord account full name']}")
        return user_details
    elif user_details:
...
def run_anonymous_checker(filename):
    try:
        with open(filename, 'r', newline='', encoding='utf-8') as file:
            ...
            if not all_data:
                logging.warning("⚠️  CSV file is empty. Cannot check for anonymous accounts.")
                return
    ...
    accounts_to_check = [row for row in all_data if str(row.get('Enkord account full name', '')).lower().startswith("anonymous#")]
    if not accounts_to_check:
        logging.info("✅ No 'Anonymous' accounts found to check.")
        return

    logging.info(f"Found {len(accounts_to_check)} 'Anonymous' accounts to re-check.")
    updated_details = {}
    with ThreadPoolExecutor(max_workers=6) as executor:
        future_to_id = {executor.submit(recheck_anonymous_user, row['User ID']): row['User ID'] for row in accounts_to_check}
        ...
        for future in as_completed(future_to_id):
            try:
                ...
                logging.error(f"❌ Worker thread failed during anonymous check for ID {future_to_id[future]}: {e}")
        
    if not updated_details:
        logging.info("⚠️  No 'Anonymous' accounts were updated after re-checking.")
        return

    logging.info(f"Found {len(updated_details)} updated names. Updating CSV file...")
...
def main_menu(filename):
    while True:
        ...
        if choice == '1':
            ...
            logging.info("✅ Scraping complete. Re-sorting and de-duplicating file...")
            sort_and_clean_csv(filename)
        elif choice == '2':
            run_anonymous_checker(filename)
            logging.info("✅ Anonymous check complete. Re-sorting and de-duplicating file...")
            sort_and_clean_csv(filename)
        elif choice == '3':
            break
        else:
